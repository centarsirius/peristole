...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 24 14:07:40 2022
...
for ang in range(0,3):
    ...
    dtr_d=-(dri/(a_bar*omega_p))
    #dtr_d= ((np.sin(eta)*np.cos(phi)-(np.cos(i[ang])*np.cos(eta)*np.sin(phi)))/(np.sin(zeta)*np.sqrt(1-np.sin(i[ang])**2*np.sin(phi)**2)))
